...
def carregar_dados():
    ...
    # Remove espaços extras dos nomes das colunas e preserva todas as linhas
        ...
        if len(status_counts) > 0:
            for status, qtd in status_counts.items(): 
                ...
                if st.button(f"🔍 Detalhes", key=f"btn_{status}"):
                    st.session_state.df_modal = df_f[df_f['CATEGORIA_COR'] == status]
...
# --- QUADRANTE SUPERIOR E INFERIOR (MANTENDO LÓGICA ANTERIOR) ---
# ... (O restante da sua lógica permanece intacto conforme solicitado) ...
